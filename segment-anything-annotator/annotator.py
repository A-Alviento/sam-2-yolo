...
# a colormap for different labels, imported from imgviz
# this is used to visualize the segmentation mask
LABEL_COLORMAP = imgviz.label_colormap()

# the main window class which is a subclass of QMainWindow
# QMainWindow is a class from PyQt5 library for building the main application window
class MainWindow(QMainWindow):

    # constants for different zoom modes
    FIT_WINDOW, FIT_WIDTH, MANUAL_ZOOM = 0, 1, 2

    # this is the constructor of the MainWindow class, it gets called when an instance of the class is created
    def __init__(self, parent=None, global_w=1000, global_h=1800, model_type='vit_b', keep_input_size=True, max_size=1080):
        # this super function calles the __init__() function of the parent class QMainWindow,
        # this allows the subclass to inherit methods and variables from the parent class
        super(MainWindow, self).__init__(parent)

        # set the size of the window
        self.resize(global_w, global_h)

        # saving parameters into the object
        # these variables will be used throughout the MainWindow class
        ...
        # create a new canvas object, which is the drawing area of the window
        # the self argument makes MainWindow the parent of the canvas
        # which means that the canvas will be contained within MainWindow
        ...
        # initialise variables and lists used throughout the class
        self._noSelectionSlot = False
        self.current_output_dir = 'output'
        os.makedirs(self.current_output_dir, exist_ok=True)
        self.current_output_filename = ''

        # connect the zoomRequest signal from the canvas to  the zoomRequest slowof the MainWindow
        # when the canvas emits zoomRequest (when user tries to zoom in or out), the zoomRequest method of the MainWindow will be called
        self.canvas.zoomRequest.connect(self.zoomRequest)

        # initilise memory_shapes and other parameters related to the SAM model
        self.memory_shapes = []
        self.sam_mask = []
        ...
        # setting up a scrollArea which will contain the canvas
        # this allows the canvas to be scrolled
        self.scroll_values = {
            Qt.Horizontal: {},
            Qt.Vertical: {},
        }
        ...
        # connect the scrollRequest signal from the canvas to the scrollRequest slot of the MainWindow
        # similar to zoomRequest, when the canvas emits scrollRequest, the scrollRequest method of MainWindow will be invoked
        self.canvas.scrollRequest.connect(self.scrollRequest)

        # connect various signals emitted by  canvas to slots of MainWindow
        # this is the way communication is done in PyQt5 between different parts of the applicaiton
        self.canvas.newShape.connect(self.newShape) # when a new shape is created, newShape signal is emitted by the canvas
        self.canvas.shapeMoved.connect(self.setDirty) # when a shape is moved, shapeMoved signal is emitted by the canvas
        self.canvas.selectionChanged.connect(self.shapeSelectionChanged) # when a shape is selected, selectionChanged signal is emitted by the canvas
        self.canvas.drawingPolygon.connect(self.toggleDrawingSensitive) # when a polygon is being drawn, drawingPolygon signal is emitted by the canvas

        # create and setup UniqueLabelQListWidget and LabelListWidget
        ...
        # LabelDialog is a custom dialog for creating and editing labels
        # it is created with no labels initially
        ...
        # LabelListWidget is a custom widget that shows the list of labels in the user interface
        self.labelList = LabelListWidget()

        # connect signals for label selection, editing, change and order to their corresponding methods
        self.labelList.itemSelectionChanged.connect(self.labelSelectionChanged)
        self.labelList.itemDoubleClicked.connect(self.editLabel)
        self.labelList.itemChanged.connect(self.labelItemChanged)
        self.labelList.itemDropped.connect(self.labelOrderChanged)
        
        # create a dockable widget in the main window that will contain the list of polygon labels
        ...
        # read the list of categories from a text file and remove any leading/trailing whitespace 
        self.category_list = [i.strip() for i in open('categories.txt', 'r', encoding='utf-8').readlines()]
        # reinititialise the label dialog with the category list
        self.labelDialog = LabelDialog(
            parent=self,
        ...
        # initialise the zoom_values dictionary
        self.zoom_values = {}

        # initialise video_directory and video_list variables for handling video data
        self.video_directory = ''
        self.video_list = []
        self.video_len = len(self.video_list)

        # initialise variables for image data management
        self.img_list = []
        ...
        # create "Next Image" and "Last Image" button and connect them to their respective methods
        ...
        # create a progress bar to show the progress of some task
        ...
        # create "Proposal" buttons and connect them to their respective slots
        ...
        # create a list of proposal buttons 
        self.button_proposal_list = [self.button_proposal1, self.button_proposal2, self.button_proposal3, self.button_proposal4]
        
        # flag for turning class on/off and a label to show the current status
        self.class_on_flag = True
        self.class_on_text = QLabel("Class On", self)
        

        # setting up a naive layout by moving and resizing widgets manually
        # better approach would be to use a layout manager by PyQt5
        ...
        # initialise a helper function for creating new actions
        action = functools.partial(utils.newAction, self)
        
        # create action for loading a category file
        categoryFile = action(
            self.tr("Category File"),
            lambda: self.clickCategoryChoose(), # method to be called when the action is triggered
            'None', # shortcut key
            "objects", # icon name
            self.tr("Category File"), # tool tip
            enabled=True,  # whether the action is enabled
        )

        # create action for selecting an image directory
        imageDirectory = action(
            self.tr("Image Directory"), # action name
            lambda: self.clickFileChoose(), # method to be called when the action is triggered
            'None',
            "objects",
            self.tr("Image Directory"), # tool tip
            enabled=True,
        )

        # create an action for loading SAM data
        LoadSAM = action(
            self.tr("Load SAM"),
        ...
        # disabled auto segmentation for now
        AutoSeg = action(
            self.tr("AutoSeg"),
        ...
        # create an action for adding (accepting) the SAM mask, initially disabled
        promptSeg = action(
            self.tr("Accept"), 
            lambda: self.addSamMask(),
            'a', # shortcut key
            "objects",
            self.tr("Accept"), # tool tip
            enabled=False, 
        )

        # create an action for chossing the save directory
        saveDirectory = action(
            self.tr("Save Directory"), # action name
            lambda: self.clickSaveChoose(), # method to be called when the action is triggered
            'None',
            "objects",
        ...
        # create an actoin for drawing manual polygons
        createMode = action(
            self.tr("Manual Polygons"),
            lambda: self.toggleDrawMode(False, createMode="polygon"), # method to be called when the action is triggered
            'Ctrl+W', # shortcut key
            "objects",
        ...
        # create an action for prompting points
        createPointMode = action(
            self.tr("Point Prompt"),
            lambda: self.toggleDrawMode(False, createMode="point"), # method to be called when the action is triggered
            'None',
            "objects",
            self.tr("Point Prompt"), 
            enabled=True,
        ...
        # create an action for prompting boxes
        ...
        # create an action to reject mask generated
        cleanPrompt = action(
            self.tr("Reject"),
        ...
        # create an action for toggling the class
        ...
        # create an action for editing polygons, initially disabled
        editMode = action(
            self.tr("Edit Polygons"), # action name
            self.setEditMode, # method to be called when the action is triggered
            'None',
            "edit",
        ...
        # create an action for saving as a different file
        saveAs = action(
            self.tr("&Save As"),
            self.saveFileAs,
            'ALT+s',
        ...
        # create an action for undoing the last point, initially disabled
        ...
        # create an action for hiding all polygons, initially disabled
        hideAll = action(
            self.tr("&Hide\nPolygons"),
        ...
        # create an action for showing all polygons, initially disabled
        showAll = action(
            self.tr("&Show\nPolygons"),
        ...
        # create an action for undoing the last shape added or edited, initially disables
        undo = action(
            self.tr("Undo"),
            self.undoShapeEdit,
            'Ctrl+U',
        ...
        # create an action for saving labels to file, initially disabled
        save = action(
            self.tr("&Save"),
            self.saveFile,
            'S',
        ...
        # create an action for deleting selected polygons, initially disabled
        delete = action(
        ...
        # create an action for duplicating selected polygons, initially disabled
        duplicate = action(
        ...
        # create an action for reducing points
        reduce_point = action(
        ...
        # create an action for editing the label of the selected polygon, initially disabled
        edit = action(
            self.tr("&Edit Label"),
            self.editLabel,
            'None',
        ...
        # create a structure for the actions
        self.actions = utils.struct(
            ...
            loadSAM=LoadSAM,
            #autoSeg=AutoSeg, # AutoSeg action is commented out
            promptSeg=promptSeg,
        ...
        # Add actions to the context menus of the canvas widget
